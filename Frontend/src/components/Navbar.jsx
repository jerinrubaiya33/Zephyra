...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div
        className={`transform transition-transform duration-300 ease-in-out ${hideNavbar ? "-translate-y-full" : "translate-y-0"}`}
      >
        {/* TOP BAR */}
        {/* //#72873A khaki*/}
        {/* //#9C7E63 Brown*/}
      ...
      {/* SIDEBAR */}
      <div
        className={`fixed top-0 right-0 h-screen w-3/4 max-w-sm bg-white z-50 shadow-lg transform transition-transform duration-300 sm:hidden ${visible ? "block translate-x-0" : "hidden translate-x-full"}`}
      >
        <button
          onClick={() => setVisible(false)}
          className="absolute top-4 right-8 text-2xl"
        ...
        <div className="flex flex-col mt-12 bg-white">
          <NavLink
            onClick={() => setVisible(false)}
            className="block bg-white p-4 border-b border-pink-300"
            to="/collection"
          >
            COLLECTION
          </NavLink>
